...
export const isConfigured = (p: Provider): boolean =>
  p === 'local' || Boolean(p === 'google' ? googleClientId : msClientId);

export const getAccount = (): Account | null => readJson<Account>(ACCOUNT_KEY);
...
export function login(p: Exclude<Provider, 'local'>, opts: { silent?: boolean; hint?: string } = {}): void {
...
export const tokenGetter = (p: Exclude<Provider, 'local'>): TokenGetter => (p === 'google' ? googleToken : msToken);

export function createDriver(a: Account): Driver {
  if (a.provider === 'local') return new LocalDriver();
...
/** Removes the offline copy of an account's data (the local-mode database itself is kept). */
